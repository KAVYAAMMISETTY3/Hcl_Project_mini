...
public class TransactionController {
    ...
    public ResponseEntity<List<TransactionResponseDTO>> getUserBorrowedBooks(@PathVariable Long userId) {
        return ResponseEntity.ok(transactionService.getUserBorrowedBooks(userId));
    }

    @GetMapping("/borrow")
    public ResponseEntity<List<TransactionResponseDTO>> getAllBorrowedBooks() {
        return ResponseEntity.ok(transactionService.getCurrentlyBorrowedBooks());
    }
}
